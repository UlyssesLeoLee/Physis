...
| v0.1.3 | 2026-08-17 | 响应 01号文档 v0.1.4 多宿主重构：R-11 范围扩大至全部宿主；新增 R-12（Tier 2 C ABI 固化风险）、R-13（宿主+GPU 双线范围失控风险）、R-14（GPU 设备共享可行性未验证）；新增 ISS-012（多宿主规划是否违反最小可验证架构原则的自审） |
...
| R-11 | 宿主快速迭代导致各适配层频繁需要跟随重写（Bevy 的 ECS/Schedule、Godot 的 GDExtension API 均有破坏性变更史），且宿主数量越多该成本越是叠加 | 中：适配层维护成本持续高于预期，或长期滞后于宿主最新版 | 高 | ADR-009/010 已将影响面限定在各适配层 crate（核心与中立契约不受影响）；PRE-ENG-007 规定声明单一主版本、随宿主发版独立发版，接受"滞后一个版本周期"为可接受状态而非缺陷；`pre-engine-api` 收敛共用逻辑后，单个宿主升级只需改其薄适配层 |
| R-12 | Tier 2 C ABI（`pre-ffi`）一经发布即难以变更，若在没有真实 Unity/Unreal 调用方的情况下过早固化接口形态，后续修正代价高 | 中：错误的 ABI 形态被外部依赖锁定 | 中 | MVP 明确只设计不实现（PRE-ENG-009）；实现触发条件设为"出现真实接入需求"而非时间表（OQ-08）；同时 ABI 约束**现在就反向作用于中立契约设计**，确保将来能接上 |
| R-13 | 宿主集成（4 类）与 GPU 后端两条线同时展开，导致 MVP 范围失控，挤占 H1~H5 假设验证的工程周期（约束 C-03：8~12 周） | 高：核心假设未验证而资源已耗尽，违背项目立论顺序 | 中高 | 判定基线严格执行——凡不影响 H1~H5 可验证性者一律不进 MVP：GPU 整体不实现（仅预留设备注入形态）、Tier 2/3 仅设计、Godot 排在 Bevy 之后且非验收前置；本风险已在 01号文档 §19 以"范围警示"形式显式标注 |
| R-14 | PRE-GPU-002 的宿主设备注入，在各宿主上导出可共享设备/队列句柄的可行性未经验证（Bevy 基于 wgpu 可行性高，Godot/Unity/Unreal 使用各自渲染后端） | 中：若多数宿主无法注入，PRE-GPU-002 的价值下降，PRE-GPU-004 零拷贝亦不可达 | 中 | 记为 OQ-10，需在 GPU 工作启动前逐宿主评估；即便不可注入，自建设备路径仍可用（PRE-GPU-002 已定义 `CreateOwn` 分支），故风险影响面限于"优化不可得"而非"功能不可用" |
...
- Recommendation: 判定为可接受的例外，理由有三：(1) 范围已被 ADR-009 严格限定为独立可选 crate，零侵入核心，不产生"污染核心架构"的实际风险；(2) 回放能力对 H1~H5 验证本身有辅助价值（可视化调试候选/观测轨迹差异，帮助人工判断检索结果是否合理，间接支撑 06 号文档的实验分析工作）；(3) 已明确将高不确定性的双向映射方向（旧 PRE-BEVY-005，现 PRE-ENG-011）推迟到 Phase 2，只在 MVP 纳入低风险的单向回放（旧 PRE-BEVY-002/003，现 PRE-ENG-004/101）。故不视为违反最小可验证架构原则，但如实记录其需求来源与其他需求不同，供后续审查参考
- Affected Requirements: PRE-ENG-*（原 PRE-BEVY-001~006）
- Affected Components: pre-bevy, pre-engine-api
- 后续更新（v0.1.3）：随需求扩展至 Godot/Unity/Unreal/DCC 四类宿主 + GPU 后端，本条的判断需重新审视，见 ISS-012

**ISS-012**
- Severity: Medium
- Impact: 多宿主分层集成（4 类宿主、4 个 Tier）与 GPU 后端的完整规划，是否违反本项目"最小可验证架构 / 未被实验观察到的问题不加抽象"原则？规划中的绝大部分（Tier 2/3、Unity/Unreal/DCC、全部 GPU 能力）在 MVP 阶段既不实现也无实验证据支撑
- Evidence: 01号文档 §29/§30 的需求来源是用户明确的产品方向要求，而非 H1~H5 实验推导；ISS-011 曾对单一 Bevy 集成做过同类判断，此次规模扩大约 5 倍
- Recommendation: 判定为**有条件可接受**，但条件比 ISS-011 更严格，理由与边界如下：
  (1) 真正在 MVP 落地的只有 `pre-engine-api` 中立契约 + Bevy 适配层，其余全部是"约束"而非"实现"——约束的成本是文档与少量接口形态，不是代码量；
  (2) 其中三条属于"早期不做、后期无法补救"的类别，具备独立于实验证据的正当性：PRE-ENG-006 坐标/单位换算（ADR-011 已论证：只做 Bevy/Godot 必然把换算做成隐式恒等，接入 Unreal 时无处插入）、PRE-GPU-002 设备注入（ADR-012 已论证：自建与注入是不兼容的初始化架构）、PRE-EMB-001/002（无全局状态、不占主循环）；
  (3) 反之，**不具备此类正当性的部分必须严格保持"不实现"**：Tier 2 C ABI 的具体函数签名、Unity/Unreal/DCC 的适配层、GPU 计算 kernel、零拷贝互操作——这些既无实验证据也无"后期不可补救"属性，一旦开始实现即构成过度设计。
- 风险联动：本条与 R-13（范围失控）是同一问题的两个视角——ISS-012 关注"是否该规划"，R-13 关注"规划后是否会失控"。二者的缓解手段一致：把"约束"与"实现"严格分开，只让前者提前。
- Affected Requirements: PRE-ENG-*, PRE-GPU-*, PRE-EMB-*
- Affected Components: pre-engine-api, pre-ffi, pre-python, pre-gpu, 各适配层
...
ISS-001, ISS-002, ISS-003, ISS-007, ISS-008, ISS-010, ISS-011, ISS-012（均为 Low/Medium，且已有明确缓解路径）
