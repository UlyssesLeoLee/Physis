...
| v0.1.4 | 2026-08-17 | 新增 ADR-013：图论构造能力是关系存储之上的逻辑层，而非重新引入图数据库——明确 ADR-006 的存储决策与图论查询能力需求（01号文档第32节）不矛盾 |
| v0.1.5 | 2026-08-17 | 新增 ADR-014（统一插件内核）、ADR-015（ECS 兼容性以数据形状契约实现，不内置 ECS 运行时） |
...
## ADR-013: 图论构造能力为何是关系存储之上的逻辑层，而非重新引入图数据库？

**状态**：Accepted

**背景**：01号文档第32节新增图论构造需求（PRE-GRAPH-001~006）。此前 ADR-006 已判定"不引入图数据库"，若不澄清二者关系，容易被误读为"该需求与既有架构决策冲突，图论能力做不了"或反过来"既然要图论能力，就该引入图数据库、推翻 ADR-006"——两种误读都会导致错误的实现方向。

**决策**：Physical Graph 是**逻辑数据模型**（节点与边的抽象），不是存储选型。节点/边完全从 `pre-atlas` 已有的关系型数据派生，作为只读视图存在；遍历查询通过 SQLite 的 `WITH RECURSIVE` 递归 CTE 实现，不引入专用图数据库，也不新增持久化系统。

**理由**：

1. **ADR-006 否决的是"图数据库作为存储后端"，从未评估过"图论查询能力"本身**。这是两个不同层面的问题：存储选型回答"数据落在哪种引擎里"，逻辑模型回答"数据以什么抽象呈现给查询者"。关系型数据库完全可以对外呈现图语义（这正是 `WITH RECURSIVE` 存在的意义），二者不是非此即彼。

2. **重新引入图数据库来满足 PRE-GRAPH-* 会违反 ADR-006 已确立的证据标准**："先证明关系型/向量存储无法满足查询模式，才可引入图数据库"（PRE-DATA-004）。PRE-GRAPH-002 要求的查询模式（有界跳数遍历、连通性判断）恰恰是递归 CTE 的标准应用场景，谈不上"无法满足"。

3. **单 Experience 范围约束（PRE-GRAPH-003）使递归 CTE 的性能特征可控**：图规模被限定在一条 Response 内的 landmark/约束/接触事件数量级（通常几十到几百个节点），远低于"关系型递归查询开始劣于原生图数据库"的规模阈值。若未来场景需要跨 Experience 的大规模图遍历，才是重新评估图数据库的合理触发点（与 ADR-006 后果条一致）。

**后果**：

- `pre-atlas` 新增一个查询子模块（非新 crate），产出图视图与遍历 API，详见 08号文档新增章节。
- 深度上限（PRE-GRAPH-006）是该实现方式下的强制安全阀——递归 CTE 若不限深度，病态输入下的行为与图数据库缺乏保护时同样危险，此风险不因"没用图数据库"而消失，必须在查询层显式处理。
- 若后续实测（06号文档 Benchmark 体系）发现递归 CTE 在目标规模下性能不达标，触发条件是"证据"而非"直觉"——按 ADR-006 的既有原则，需要先有性能数据，再考虑重新引入图数据库，不得跳过验证直接切换存储技术。


---

## ADR-014: 为何统一插件内核，而非让每类插件点各自为政？

**状态**：Accepted

**背景**：项目发展至此已积累五类独立的"可插拔"扩展点——`SolverPlugin`（ADR-003 引入）、`ParamOptimizer`（02号文档 §14）、`ObservationBackend`（02号文档 §15）、`QueryExecutor`（ADR-010 引入）、`GraphExporter`（ADR-013 引入）。审查发现它们是在不同时间点、针对不同问题独立设计的，均未定义生命周期与失败回滚语义——`ObservationBackend` 甚至没有 `unload` 的概念。

**决策**：新增统一插件生命周期状态机与 `PluginRegistry`（02号文档 §37），现有五类插件 trait 在其上做**向后兼容的特化**（补齐生命周期钩子，不改动已有领域方法签名）。

**理由**：

1. **不一致的失败处理是真实的运维风险，不是理论问题**。若 `SolverPlugin` 初始化失败时的清理逻辑与 `ObservationBackend` 初始化失败时的清理逻辑各写一套，任何一处遗漏都会导致资源泄漏或状态残留，且这类 bug 通常只在长时间运行或反复插拔插件的场景下才暴露——正是"底层可嵌入运行时"最常见的使用模式（宿主反复热重载）。
2. **回滚语义必须是状态机的一等公民，不能是事后补丁**。`Initializing → Failed` 不应止步于"标记失败"，必须显式转换到"已回滚、等价于从未注册"这一可验证的目标状态（AC-08 是其验收标准）。若不作为状态机设计的一部分预先规划，日后为某类插件"补上"回滚逻辑的改动会散落在调用方代码各处。
3. **不推翻已有 trait 签名的约束是刻意的**：08 号文档已就 `SolverPlugin`/`QueryExecutor`/`GraphExporter` 的具体接口做了详细设计并已合并，重新设计这些签名的成本与收益不成正比。统一内核以"补齐生命周期钩子"的方式介入，是对既有工作的复用而非否定。

**后果**：
- `pre-core` 新增 `PluginLifecycle` trait 与 `PluginRegistry` 类型，成为全部插件类型的公共基础设施。
- 五类既有插件的 08 号文档章节需要在实现阶段补充生命周期钩子的具体实现，但接口层面不破坏性变更。
- `PRE-PLUGIN-006`（能力查找）刻意不要求 MVP 落地——统一生命周期是地基，能力查找是地基之上的进一步优化，二者优先级不同，捆绑要求会不必要地扩大 MVP 范围。

---

## ADR-015: ECS 兼容性为何以数据形状契约实现，而非内置 ECS 运行时？

**状态**：Accepted

**背景**：Bevy（纯 ECS）与 Godot（可桥接为类 ECS）是 Tier 1 目标宿主，Unity/Unreal 虽非 ECS 但有同构的 Actor/GameObject-Component 模型。若 PRE 核心内部状态管理直接采用某个 ECS 框架（如把 `pre-core` 的内部存储实现为 `bevy_ecs::World`），可以省去适配层的映射代码，但会绑定核心与该框架的演进节奏。

**决策**：PRE 核心不依赖任何具体 ECS 框架实现；提供 `ComponentView` 系列契约（02号文档 §38.2），定义"PRE 数据 → 类 Component 数据片段"的标准映射规则，运行时集成（无论拉模式还是推模式）由各 Tier 1/2 适配层基于该契约实现。

**理由**：

1. **与 ADR-009/010 同一立场的必然延伸**：核心不依赖宿主 SDK 的原则，对"ECS 框架"同样适用——`bevy_ecs` 本身就是 Bevy 生态的一部分，若核心依赖它，等于核心间接依赖了 Bevy，与"适配层零依赖"的既有决策直接冲突。
2. **"ECS 兼容"与"内置 ECS 运行时"是两个不同粒度的需求，本项目只需要前者**。用户要求的兼容性，落到实处是"PRE 的数据能顺畅地在 ECS 宿主里当作 Component 使用"，这只需要数据形状正交、映射规则清晰（`ComponentView`），不需要 PRE 自己运行一套 ECS 调度。
3. **非 ECS 宿主（Unreal/Unity）反过来验证了该决策的必要性**：若 `ComponentView` 的设计假设"宿主必须是 ECS"，Unreal/Unity 将无法复用同一套契约——`PRE-ECS-004` 明确要求映射规则建立在"实体关联正交数据片段"这一更弱的前提上，这正是不绑定具体 ECS 框架带来的可移植性收益。

**后果**：
- `pre-engine-api` 新增 `ComponentView` trait，Tier 1 适配层的既有组件（如 `pre-bevy` 的 `PreLandmark`，08号文档 §19）需在实现阶段对齐到该标准命名与拆分规则。
- 拉/推双模式（PRE-ECS-003）不新增机制，是既有 `PlaybackCursor`（拉）/`QuerySession`（推）在"数据形状"维度的正交补充，两个设计维度互不干扰。
- 若未来出现 PRE 核心自身需要类 ECS 查询能力的真实场景（而非仅为适配层服务），应作为独立需求重新评估，不应把"核心不内置 ECS 运行时"的决策与"核心内部能否有类似 ECS 的组织方式"混为一谈——本 ADR 只约束对外契约，不约束核心内部实现细节。
